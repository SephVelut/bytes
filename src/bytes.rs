...
impl Bytes {
    ...
    ///
    /// split_1.merge(split_2);
    /// assert_eq!(b"aaabbb", &split_1[..]);
    /// ```
    pub fn merge(&mut self, other: Bytes) {
        if self.is_empty() {
            *self = other;
            return;
        }

        if let Err(other) = self.unsplit(other) {
    ...
    /// Absorbs a contiguous `Bytes` that was previously split off.
    ///
    /// If the two `Bytes` objects were previously contiguous, i.e.,
    /// if `other` was created by calling `split_off` or `split_to` on
    /// the same `BytesMut`, then this is an `O(1)` operation that
    ...
    /// assert_eq!(b"aaabbb", &split_1[..]);
    /// ```
    pub fn unsplit(&mut self, other: Bytes) -> Result<(), Bytes> {
        if self.is_empty() {
            *self = other;
            return Ok(());
        }
